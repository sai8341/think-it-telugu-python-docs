...
<Quiz
  question="Why is multithreading highly effective for downloading 20 web pages simultaneously despite Python's GIL?"
  options={[
    "Because the GIL makes the CPU run 10x faster",
    "Because Python releases the GIL while threads wait on network I/O responses",
    "Because threads do not use memory",
    "Because web pages disable the operating system kernel"
  ]}
  correctIndex={1}
  explanation="During network and disk I/O operations, Python releases the Global Interpreter Lock, allowing multiple threads to wait on external responses simultaneously."
/>
