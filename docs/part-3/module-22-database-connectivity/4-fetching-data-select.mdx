...
<Quiz
  question="What setting enables column name access (e.g., row['email']) instead of numerical tuple indices (row[1]) in Python SQLite3?"
  options={[
    "conn.enable_dict()",
    "conn.row_factory = sqlite3.Row",
    "cursor.set_mode('dict')",
    "sqlite3.use_json_names()"
  ]}
  correctIndex={1}
  explanation="Assigning sqlite3.Row to conn.row_factory allows returned rows to be accessed both by numerical index and by column name string."
/>
