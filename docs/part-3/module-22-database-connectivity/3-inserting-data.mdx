...
<Quiz
  question="Why must you always use question mark ? placeholders instead of f-strings when building SQL insert queries?"
  options={[
    "Because SQLite crashes on f-strings",
    "To prevent dangerous SQL Injection attacks by letting the database engine safely sanitize all inputs",
    "Because question marks make the text lowercase",
    "To compress the file size by 50%"
  ]}
  correctIndex={1}
  explanation="Using ? parameter substitution ensures that user input is treated strictly as data rather than executable SQL commands, completely neutralizing SQL Injection vulnerabilities."
/>
