...
1. If you execute `s2.name = "Ananya"`, what will `print(s1.name)` output?
2. What is the difference between `s1 == s2` and `s1 is s2` in Python?
3. What error occurs if you try to access `s1.non_existent_attribute`?
4. How many objects exist in memory after executing `a = Student()`, `b = Student()`, `c = a`?

---

## 12. Mini Exercises

### Exercise 2.1: Reference Mutation Challenge
...
Explain what happened in Heap memory.

### Exercise 2.2: The `id()` Function
Python has a built-in function `id(obj)` that returns the integer memory address of an object. Create two objects `x = Phone()` and `y = Phone()`. Print `id(x)` and `id(y)`. Are they equal? Now set `z = x` and print `id(z)`. Which ID does it match?
