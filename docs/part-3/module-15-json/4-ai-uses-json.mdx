...
<Quiz
  question="Which built-in exception does Python raise when you attempt to parse a corrupted or invalid JSON string with json.loads()?"
  options={[
  ...
  correctIndex={0}
  explanation="When json.loads() or json.load() encounters invalid JSON syntax, it raises json.JSONDecodeError (which inherits from ValueError)."
/>
