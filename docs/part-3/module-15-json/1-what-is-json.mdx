...
* **Double Quote Rule:** JSON strictly requires double quotation marks (`"key": "value"`) for all keys and strings. Single quotes are invalid.
* **Data Type Mappings:** `true` -> `True`, `false` -> `False`, `null` -> `None`, Array `[]` -> `list`, Object `{}` -> `dict`.
