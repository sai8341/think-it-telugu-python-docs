---
title: "Module 14: File Handling Introduction - Python in Telugu | Think IT Telugu"
...
1. **[What is a File?](./what-is-a-file)**: Introduction to files, text files, and binary files.
2. **[Reading Files](./reading-files)**: How to read file contents using `read()`, `readline()`, and `readlines()`.
3. **[Writing & Appending](./writing-appending)**: How to write data to new files and append to existing files using `w` and `a` modes.
4. **[Safe File Opening (with open)](./reading-files#the-with-statement-recommended-way)**: Using the `with` statement to ensure files are closed automatically and safely.
