...
* **Context Manager (`with open(...) as f:`):** The standard Python best practice that automatically closes the file upon exit.
* **`file.read()`:** Reads the entire file content into a single string in memory.
* **`file.readline()`:** Reads one line at a time from current cursor position.
* **`file.readlines()`:** Loads all lines as a list of strings.
* **Streaming Line-by-Line:** `for line in file:` streams large multi-gigabyte files efficiently without RAM exhaustion.
