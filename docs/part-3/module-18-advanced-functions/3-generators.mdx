...
<Quiz
  question="What happens to the internal local variables of a generator function when it hits a yield statement?"
  options={[
    "They are wiped from memory completely",
    "They are frozen and saved in place until next() is called",
    "They are converted into global variables",
    "The function restarts from line 1"
  ]}
  correctIndex={1}
  explanation="When Python encounters yield, it pauses execution and freezes all local variables and instruction pointers, allowing execution to resume exactly where it stopped on the next iteration."
/>
