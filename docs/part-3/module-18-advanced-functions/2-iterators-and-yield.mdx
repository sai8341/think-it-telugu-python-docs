...
<Quiz
  question="What built-in exception is raised when next() is called on an exhausted iterator with no default value?"
  options={["IndexError", "StopIteration", "ValueError", "KeyError"]}
  correctIndex={1}
  explanation="When an iterator has no more items remaining, Python raises StopIteration to signal the iteration loop to terminate."
/>
