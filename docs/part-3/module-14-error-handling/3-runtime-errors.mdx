...
<Quiz
  question="What exception does Python raise when you try to access a dictionary key that does not exist?"
  options={["IndexError", "ValueError", "KeyError", "NameError"]}
  correctIndex={2}
  explanation="KeyError is raised when you access a key that is not present in the dictionary. IndexError is for lists, ValueError is for invalid values, and NameError is for undefined variables."
/>
