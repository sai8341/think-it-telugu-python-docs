...
* **SyntaxError:** Occurs when code violates Python grammar rules (missing colons, mismatched quotes, unclosed parentheses).
* **IndentationError:** A specific subtype of syntax error caused by improper code block indentation.
* **Zero-Execution:** Python refuses to execute any part of a file if even a single syntax error is detected during parsing.
* **Fixing Strategy:** Inspect the caret marker (`^`) in the traceback, check the preceding line, and use IDE linting.

---

## What's Next?

Now that you understand compile-time grammar mistakes, let's explore errors that happen while the program is actively running: **Runtime Errors (Exceptions)**!
