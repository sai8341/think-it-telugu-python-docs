...
<Quiz
  question="What happens when Python finds a syntax error in your code?"
  options={["Python fixes it automatically", "Python runs the program but skips the broken line", "Python refuses to run even a single line of the program", "Python converts it to a warning"]}
  correctIndex={2}
  explanation="A syntax error means the code grammar is broken. Python cannot understand the instructions at all, so it refuses to execute any part of the program."
/>
