...
<Quiz
  question="Which regex shortcut matches any single digit (0 through 9) in Python?"
  options={["\\w", "\\s", "\\d", "\\D"]}
  correctIndex={2}
  explanation="\\d represents digits [0-9]. (Note: \\w is word characters, \\s is whitespace, and \\D is non-digits)."
/>
