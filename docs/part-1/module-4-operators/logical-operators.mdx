...
* **`not` Operator:** Reverses the truth value (`not True` -> `False`; `not False` -> `True`).
* **Short-Circuiting:** Python stops checking further conditions once the final boolean outcome is already determined.
