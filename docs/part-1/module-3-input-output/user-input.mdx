...
* **`input()` Function:** Prompts the user and waits for them to type an answer and press Enter.
* **Always Returns String:** Input data is always captured as text (`str`), even if numbers are typed.
* **Type Casting Numbers:** Wrap `input()` inside `int(input(...))` or `float(input(...))` to perform math calculations.
* **Inline Prompt:** Provide clear instructions directly inside `input("Enter your name: ")`.

---

## What's Next?

Now that we know how to print output and accept user input, let's look at the most powerful, modern way to format dynamic text using **f-strings** in the next lesson!
