...
* **Dict Comprehension:** `{key_expr: value_expr for item in iterable}` creates mappings dynamically in one line.
* **Set Comprehension:** `{expr for item in iterable}` creates deduplicated sets in one line.
* **`enumerate(iterable)`:** Yields `(index, item)` pairs simultaneously during iteration.
* **`zip(list1, list2)`:** Combines corresponding items from multiple sequences in lockstep.
