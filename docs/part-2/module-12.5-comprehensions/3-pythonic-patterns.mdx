...
| **Swapping** | Swap two values | `a, b = b, a` |
| **`any()` / `all()`** | Quick True/False checks | `any(x > 10 for x in nums)` |
| **`join()`** | Combine list into string | `" ".join(["a", "b"])` |
| **`sorted(key=)`** | Smart sorting | `sorted(data, key=len)` |
| **`map()` / `filter()`** | Transform or filter data | `list(map(func, data))` |
