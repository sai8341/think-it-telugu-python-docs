...
* **`KeyError` on Missing Keys:** Looking up a key that does not exist (`user["phone"]`) crashes Python.
* **Fix:** Use `"phone" in user` to check first, or use `.get("phone")` for safe lookup.
:::
...
* **Dictionary Definition:** A collection of `key: value` mappings enclosed in curly braces `{...}`.
* **Key Rules:** Keys must be unique and immutable (strings, integers, tuples); values can be any data type.
* **Access & Mutation:** Access using `dict[key]`; create or update values using `dict[key] = new_value`.
* **Fast Lookup:** Dictionaries use hash tables for lightning-fast $O(1)$ key lookups.
